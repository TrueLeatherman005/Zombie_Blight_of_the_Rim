...
namespace ZombieBlight
{
    public class ZombieThreatGraph
    {
        ...
        {
            // history теперь будет List<KeyValuePair<int, float>> благодаря изменению в ZombieTrendCalculator
            ...
            // История
            for (int i = 1; i < history.Count(); i++) // .Count() для Linq; history.Count без Linq
            {
                // Используем .Key и .Value для доступа к тику и значению
            ...
            // Прогноз
            int lastHistoryTick = history.Last().Key; // Теперь history.Last() возвращает KeyValuePair
            // Изменено: forecast.Length на forecast.Count
            for (int i = 1; i < forecast.Count; i++)
            {
                // Используем lastHistoryTick как отправную точку для прогноза
                // График ожидает шаги прогноза в GenDate.TicksPerHour
        ...
        private void HandleTooltip(Rect rect)
        {
            ...
            int daysPassed = (tickAtMouse - Find.TickManager.TicksGame) / GenDate.TicksPerDay;
            string tooltip = $"День {daysPassed:+0;-0}\n";
        ...
        private float GetThreatAtTick(int tick)
        {
            ...
            if (tick <= currentTick)
            {
                var history = trendCalculator.GetHistory(); // history теперь List<KeyValuePair<int, float>>
                var closest = history
                    .OrderBy(kvp => Mathf.Abs(kvp.Key - tick))
                    .FirstOrDefault();

                // Добавим проверку на случай, если история пуста
                if (closest.Equals(default(KeyValuePair<int, float>)) && history.Any())
                {
                    // Это может случиться, если .FirstOrDefault() не нашел элемент в пустой коллекции
                    // но мы уже проверили history.Any()
                    // Однако, если history пуста, FirstOrDefault() вернет default(KeyValuePair<int, float>)
                    // которое будет {0, 0f}. Лучше обработать случай пустой истории до FirstOrDefault.
                    // Если история пуста, возможно, вернуть базовое значение или 0f.
                    // Для простоты пока оставим как есть, но имейте в виду этот момент.
                }
                if (history.Any())
                {
                    // Если история не пуста, FirstOrDefault всегда что-то вернет.
                    // Если tick очень далек от любого тика в истории, closest будет самый близкий.
                    return closest.Value;
                }
                else
                {
                    // Если история пуста, возвращаем 0 или какое-то базовое значение по умолчанию.
                    return 0f; // Или baseLine, если это логичнее
                }
            ...
            {
                var forecast = trendCalculator.GetForecast(); // forecast - List<float>
                int hourIndex = (tick - currentTick) / GenDate.TicksPerHour; // Рассчитываем индекс часа относительно текущего времени
                                                                             // Исправлено: forecast.Length на forecast.Count
                if (hourIndex >= 0 && hourIndex < forecast.Count)
                    return forecast[hourIndex];
            }

            // Если тик не в истории и не в прогнозе, возвращаем 0 или другое значение по умолчанию
            return 0f;
        }
    }
}